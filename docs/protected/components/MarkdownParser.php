...
class MarkdownParser extends CMarkdownParser
{
	...
	{
		Yii::import('ext.geshi.GeSHiHighlighter');
		$lang = current(preg_split('/\s+/', substr($options, 1, -1), 2));
		$highlighter = new GeSHiHighlighter();
		$highlighter->language = $lang;
		$highlighter->showLineNumbers = $this->getInlineOption('showLineNumbers', $options, false);
		return $highlighter;
	}
}